...
with tab1:
    ...
    # --- 🐟 魚種と匹数・サイズのダイナミック入力エリア ---
    st.subheader("🐟 釣果の記録（魚種ごとに詳細を記録！）")
    selected_targets = st.multiselect("釣れた魚種を選んでね", ["メバル", "アオリイカ", "アコウ", "アジ", "カサゴ", "シーバス", "その他（下に入力）"])
    
    target_results = []
    overall_max_size = 0.0  # 全体の中での最大サイズを自動計算するための準備
    
    if selected_targets:
        cols = st.columns(2) # スマホで押しやすいように2列で綺麗に並べる
        for i, target in enumerate(selected_targets):
            with cols[i % 2]:
                if target == "その他（下に入力）":
                    other_name = st.text_input("魚種名", key=f"name_{i}")
                    count = st.number_input("匹数", min_value=1, step=1, key=f"count_other_{i}")
                    size = st.number_input("最大(cm)", min_value=0.0, step=0.5, key=f"size_other_{i}")
                    if other_name:
                        target_results.append(f"{other_name}({count}尾・MAX{size}cm)")
                        if size > overall_max_size: overall_max_size = size
                else:
                    st.markdown(f"**🐟 {target}**")
                    count = st.number_input("匹数", min_value=1, step=1, key=f"count_{target}_{i}")
                    size = st.number_input("最大(cm)", min_value=0.0, step=0.5, key=f"size_{target}_{i}")
                    target_results.append(f"{target}({count}尾・MAX{size}cm)")
                    if size > overall_max_size: overall_max_size = size
    ...
    if submit_button:
        ...
        if final_location == "":
            ...
            # size 変数の代わりに overall_max_size をスプレッドシートに送る
            row_data = [
                str(date), time_str, final_location, final_target, 
                tide, final_tide_movement, bait, overall_max_size, memo, image_url
            ]
